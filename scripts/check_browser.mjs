...
const photoSources = await Promise.all((await readdir('_portfolio')).filter(name => /\.(md|html)$/.test(name)).map(name => readFile('_portfolio/' + name, 'utf8')));
...
try {
  for (const colorScheme of ['light', 'dark']) {
    for (const width of [1365, 390]) {
      ...
      for (const path of ['/', '/publications/', '/cv/', '/portfolio/', '/talks/', '/research/']) {
  ...
  console.log('Passed: 24 desktop/mobile page renders, light/dark menus, keyboard theme control, persistence, images, analytics consent, publication equations and real map markers.');
